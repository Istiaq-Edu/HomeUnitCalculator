...
class MainTab(QWidget):
    ...
    def load_info_to_inputs_from_csv(self, selected_month, selected_year):
        ...
        try:
            with open(filename, mode='r', newline='', encoding='utf-8') as file:
                ...
                found_main = False
                # Room data handling will be simplified for now, focus on main tab elements.
                # If RoomsTab were fully refactored, we'd call a method on self.main_window.rooms_tab_instance
                ...
                for row in reader:
                    ...
                    if not csv_month_year_str: continue
                    ...
                    if selected_month_year_str_ui in csv_month_year_str : # Simple check
                        if not found_main:
                            ...
                            meter_values_csv = [get_csv_value(row, f"Meter-{i+1}", "0") for i in range(10)]
                            diff_values_csv = [get_csv_value(row, f"Diff-{i+1}", "0") for i in range(10)]
                            ...
                            found_main = True
                            # Room data loading would ideally be delegated to RoomsTab here
                            # For now, we skip complex room data loading in MainTab's isolated version.
                            # Example: if hasattr(self.main_window, 'rooms_tab_instance'):
                            # self.main_window.rooms_tab_instance.load_rooms_from_csv_row(row)
    ...
    def load_info_to_inputs_from_supabase(self, selected_month, selected_year):
        ...
        try:
            ...
            meter_values_db = [
                str(main_data.get("meter1_reading", "") or "0"),
                str(main_data.get("meter2_reading", "") or "0"),
                str(main_data.get("meter3_reading", "") or "0")
            ]
            diff_values_db = [
                str(main_data.get("diff1", "") or "0"),
                str(main_data.get("diff2", "") or "0"),
                str(main_data.get("diff3", "") or "0")
            ]

            extra_meter_readings_json = main_data.get("extra_meter_readings")
            if extra_meter_readings_json:
                try: meter_values_db.extend(map(str, json.loads(extra_meter_readings_json)))
                except json.JSONDecodeError: print("Error decoding extra_meter_readings_json")
            
            extra_diff_readings_json = main_data.get("extra_diff_readings")
            if extra_diff_readings_json:
                try: diff_values_db.extend(map(str, json.loads(extra_diff_readings_json)))
                except json.JSONDecodeError: print("Error decoding extra_diff_readings_json")
            ...
            for i, val_str in enumerate(meter_values_db):
                if i < len(self.meter_entries): self.meter_entries[i].setText(val_str)
            for i, val_str in enumerate(diff_values_db):
                if i < len(self.diff_entries): self.diff_entries[i].setText(val_str)
            
            self.additional_amount_input.setText(str(main_data.get("additional_amount", "") or "0"))
            
            # Room data would be fetched and populated by RoomsTab
            # Example: if hasattr(self.main_window, 'rooms_tab_instance'):
            # self.main_window.rooms_tab_instance.load_rooms_from_supabase(main_data.get('id'))
            
            QMessageBox.information(self, "Load Successful", f"Data for {selected_month} {selected_year} loaded from Cloud.")

        except Exception as e: # Catch Supabase APIError and others
...
if __name__ == '__main__':
    ...
    class DummyMainWindow(QMainWindow):
        def __init__(self):
            ...
            self.setup_navigation = lambda: print("DummyMainWindow.setup_navigation called")
            self.check_internet_connectivity = lambda: True # Assume connected for test

    dummy_main = DummyMainWindow()
    main_tab_widget = MainTab(dummy_main)
    main_tab_widget.setWindowTitle("MainTab Test")
    main_tab_widget.setGeometry(100, 100, 900, 700)
