...
os.environ.setdefault("QFLUENTWIDGETS_DISABLE_TIPS", "1")
...
def compare_results(current, previous):
    ...
    for key, label in metrics:
        ...
        if prev_val > 0:
            ...
            print(
                f"{label:<30} {prev_val:>10.3f}s {curr_val:>10.3f}s {change:>+10.3f}s {percent:>+6.1f}% {symbol}"
            )
        else:
...
def main():
    ...
    previous_measurement = (
        results_data["measurements"][-1] if results_data["measurements"] else None
    )
