...
class ConfirmDialog(ResponsiveDialog):
    ...
    def __init__(
        ...
        content_widget.setStyleSheet(
            "background-color: #2b2b2b; border-bottom-left-radius: 8px; border-bottom-right-radius: 8px;"
        )
        ...
        if icon_type == "question":
            icon_label.setPixmap(
                FluentIcon.INFO.icon(color=QColor(108, 92, 231)).pixmap(48, 48)
            )
        elif icon_type == "warning":
            icon_label.setPixmap(
                FluentIcon.DELETE.icon(color=QColor(198, 40, 40)).pixmap(48, 48)
            )
    ...
    def _create_title_bar(self, layout, title):
        ...
        icon_label = QLabel()
        icon_label.setPixmap(
            FluentIcon.INFO.icon(color=QColor(108, 92, 231)).pixmap(18, 18)
        )
        ...
        title_bar.mouseMoveEvent = (
            lambda e: self.move(e.globalPos() - self._drag_pos)
...
class RentalRecordDialog(ResponsiveDialog):
    ...
    def __init__(
        ...
        is_archived_record=False,
        main_window_ref=None,
        current_source="Local DB",
        supabase_id=None,
    ):
        super().__init__(parent)
        ...
        try:
            ...
            if os.path.exists(icon_path):
                ...
                if not pixmap.isNull():
                    ...
                    app_icon_label.setPixmap(
                        FluentIcon.DOCUMENT.icon(color=QColor(108, 92, 231)).pixmap(
                            18, 18
                        )
                    )
            else:
    ...
    def _create_horizontal_sections(self, layout):
        ...
        tenant_name_container.setGraphicsEffect(
            None
        )  # Remove shadow to avoid dotted edges
        ...
        tenant_name_title.setStyleSheet(
            "color:#FFFFFF; font-weight:bold; background:transparent; border:none;"
        )
        tenant_name_layout.addWidget(tenant_name_title)
        self.tenant_name_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self.tenant_name_label.setStyleSheet(
            "color:#FFFFFF; font-weight:bold; background:transparent; border:none; font-size:14px;"
        )
        ...
        room_number_container.setGraphicsEffect(
            None
        )  # Remove shadow to avoid dotted edges
        ...
        room_number_title.setStyleSheet(
        ...
        self.room_number_label.setStyleSheet(
            "color:#4FC3F7; font-weight:bold; background:transparent; border:none; font-size:14px;"
        )
        ...
        advanced_paid_container.setGraphicsEffect(
            None
        )  # Remove shadow to avoid dotted edges
        ...
        advanced_paid_title.setStyleSheet(
            "color:#81C784; font-weight:bold; background:transparent; border:none;"
        )
        advanced_paid_layout.addWidget(advanced_paid_title)
        self.advanced_paid_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self.advanced_paid_label.setStyleSheet(
            "color:#81C784; font-weight:bold; background:transparent; border:none; font-size:14px;"
        )
        ...
        created_at_container.setGraphicsEffect(
            None
        )  # Remove shadow to avoid dotted edges
        ...
        created_at_title.setStyleSheet(
            "color:#FFB74D; font-weight:bold; background:transparent; border:none;"
        )
        created_at_layout.addWidget(created_at_title)
        self.created_at_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self.created_at_label.setStyleSheet(
            "color:#FFB74D; font-weight:bold; background:transparent; border:none; font-size:14px;"
        )
        ...
        updated_at_container.setGraphicsEffect(
            None
        )  # Remove shadow to avoid dotted edges
        ...
        updated_at_title.setStyleSheet(
            "color:#EC407A; font-weight:bold; background:transparent; border:none;"
        )
        updated_at_layout.addWidget(updated_at_title)
        self.updated_at_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self.updated_at_label.setStyleSheet(
            "color:#EC407A; font-weight:bold; background:transparent; border:none; font-size:14px;"
        )
    ...
    def _create_action_buttons(self, layout):
        ...
        self.dialog_save_pdf_btn.setIcon(
            FluentIcon.DOCUMENT.icon(color=QColor(255, 255, 255))
        )
        ...
        self.dialog_archive_btn.setIcon(
            FluentIcon.FOLDER.icon(color=QColor(255, 255, 255))
        )
        ...
        self.dialog_delete_btn.setIcon(
    ...
    def display_record_details(self):
        ...
        try:
            ...
            adv_val = None
        self.advanced_paid_label.setText(
            f"{adv_val:.2f} TK" if adv_val is not None else "N/A"
        )
    ...
    def _load_network_images_parallel(self, network_urls):
        ...
        logging.info(
            f"⚡ Starting parallel fetch of {len(urls_to_fetch)} thumbnails..."
        )
    ...
    def generate_pdf_from_dialog(self):
        ...
        if self.main_window and hasattr(self.main_window, "rental_info_tab_instance"):
            if hasattr(
                self.main_window.rental_info_tab_instance,
                "generate_rental_pdf_from_data",
            ):
                pdf_path = self.main_window.rental_info_tab_instance.generate_rental_pdf_from_data(
                    self.record_data
                )
                ...
                QMessageBox.critical(
                    self, "Error", "PDF generation function not accessible."
                )
        else:
            QMessageBox.critical(
                self, "Error", "PDF generation function not accessible."
        ...
        try:
            ...
            if self.current_source == "Local DB":
                ...
                self.db_manager.execute_query(
                    update_query,
                    (
                        1 if new_archive_status else 0,
                        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        self.record_data.id,
                    ),
                )
                QMessageBox.information(
                    self, "Success", f"Record has been {action_text} in local database."
                )
            elif self.current_source == "Cloud (Supabase)":
                if (
                    self.supabase_manager
                    and self.supabase_manager.is_client_initialized()
                ):
                    ...
                    if success:
                        QMessageBox.information(
                            self,
                            "Success",
                            f"Record has been {action_text} in the cloud.",
                        )
                    else:
                        QMessageBox.critical(
                            self,
                            "Supabase Error",
                            "Failed to update record in Supabase.",
                        )
                        return
                else:
                    QMessageBox.warning(
                        self, "Supabase Error", "Supabase client not configured."
                    )
                    return
            ...
            if self.main_window and hasattr(self.main_window, "_emit_local_change"):
                self.main_window._emit_local_change(
                    ...
                    "archive_toggled",
                    {
                        "supabase_id": self.supabase_id or self.record_data.supabase_id,
                        "room_number": self.record_data.room_number,
                        "is_archived": self.is_archived_record,
                    },
                )
        ...
        if confirmed:
            try:
                if self.current_source == "Local DB":
                    ...
                    self.db_manager.execute_query(
                        "DELETE FROM rentals WHERE id = ?", (self.record_data.id,)
                    )
                    QMessageBox.information(
                        self, "Success", "Record deleted from local database."
                    )
                    ...
                    if self.supabase_manager and self.record_data.supabase_id:
                        success = self.supabase_manager.delete_rental_record(
                            self.record_data.supabase_id
                        )
                        if success:
                            QMessageBox.information(
                                self, "Success", "Record deleted from Supabase."
                            )
                            ...
                            QMessageBox.critical(
                                self,
                                "Cloud Error",
                                "Failed to delete record from Supabase.",
                            )
                            return
                    else:
                        QMessageBox.warning(
                            self, "Supabase Error", "Supabase manager not available."
                        )
                        return
                ...
                if self.main_window and hasattr(self.main_window, "_emit_local_change"):
                    self.main_window._emit_local_change(
                        ...
                        "deleted",
                        {
                            ...
                            or self.record_data.supabase_id,
                            "room_number": self.record_data.room_number,
                        },
                    )
    ...
    def _is_safe_path(self, file_path):
        ...
        try:
            ...
            safe_dirs_abs_lower = [
                os.path.abspath(d).lower() if os.name == "nt" else os.path.abspath(d)
                for d in safe_dirs
            ]
            is_in_safe_dir = any(
            ...
            in_forbidden = any(
                abs_path_lower.startswith(f.lower()) for f in forbidden_dirs
            )
