...
from src.ui.custom_widgets import (
    ...
    SmoothTableWidget,
)  # Avoid top-level import to keep optional
...
class ArchivedInfoTab(QWidget, EnhancedTableMixin):
    ...
    def init_ui(self):
        ...
        source_menu.addAction(
            Action(
                FluentIcon.SAVE,
                "Local DB",
                triggered=lambda: _set_archived_source(
    ...
    def _get_selected_record_key(self):
        try:
            row = self.archived_records_table.currentRow()
            if row < 0:
                return None
            item = self.archived_records_table.item(row, 0)
            if not item:
                return None
    ...
    def _restore_record_selection(self, record_key):
        ...
        for row in range(self.archived_records_table.rowCount()):
            item = self.archived_records_table.item(row, 0)
            if not item:
                continue
            if self._record_key_from_payload(item.data(Qt.UserRole)) == record_key:
                self.archived_records_table.selectRow(row)
                self.archived_records_table.setCurrentCell(row, 0)
                return
    ...
    def load_archived_records(self):
        ...
        if selected_source == "Local DB":
            ...
            try:
                ...
                logging.error(
                    f"Database Error: Failed to load archived rental records from local DB: {e}",
                    exc_info=True,
                )
                QMessageBox.critical(
                    self,
                    "Local DB Error",
                    f"Failed to load archived rental records from local DB: {e}",
                )
            return
    ...
    def _on_archived_cloud_ready(self, records, source):
        ...
        self._populate_archived_table(
            source,
            records,
            restore_selection_key=self._get_selected_record_key(),
        )
    ...
    def _on_archived_cloud_error(self, message: str):
        ...
        if message == "PAUSED_PROJECT":
            ...
            QMessageBox.critical(
                self,
                "Cloud DB Error",
                f"Failed to load archived rental records from Supabase: {message}",
            )
    ...
    def _populate_archived_table(
        self, source_label: str, records: list, restore_selection_key=None
    ):
        if not records:
            # Message already displayed by caller (load_archived_records or worker callback)
            return
        ...
        for row_idx, record in enumerate(records):
            ...
            self.archived_records_table.item(row_idx, 0).setData(
    ...
    def _set_intelligent_column_widths(self, table: SmoothTableWidget):
        ...
        if hasattr(table, "_layout_stabilized") and table._layout_stabilized:
            print(
                f"[ARCHIVED DEBUG] Table layout is stabilized, skipping complex resize"
            )
            return
        ...
        for col in range(column_count):
            ...
            if any(keyword in header_lower for keyword in ["tenant", "name"]):
                ...
                for keyword in ["advanced", "paid", "total", "cost", "bill", "amount"]
            ):
        ...
        print(
            f"[ARCHIVED DEBUG] Content widths: {[content_widths[col] for col in range(column_count)]}"
        )
        ...
        print(
            f"[ARCHIVED DEBUG] Content fits: {content_fits} ({total_min_width} <= {available_width - 30})"
        )
        ...
        if content_fits and available_width > 200:
            ...
            print(
                f"[ARCHIVED DEBUG] Applied STRETCH mode - content fits in {available_width}px"
            )
        else:
            ...
            print(
                f"[ARCHIVED DEBUG] Applied FIXED mode with scrolling - total width {total_min_width}px"
            )
        ...
        for col in range(column_count):
            ...
            print(
                f"[ARCHIVED DEBUG] Column {col} ({table.horizontalHeaderItem(col).text() if table.horizontalHeaderItem(col) else 'N/A'}): {mode_name}, width: {width}"
            )
    ...
    def _ensure_stretch_mode(self, table):
        ...
        try:
            ...
            if header and table.columnCount() > 0:
                print(
                    f"[ARCHIVED DEBUG] Applying proportional column distribution for {table.columnCount()} columns"
                )
                ...
                for col in range(table.columnCount()):
                    if col in column_widths:
                        ...
                        print(
                            f"[ARCHIVED DEBUG] Column {col}: fixed width {column_widths[col]}px"
                        )
                    else:
                ...
                for col in range(table.columnCount()):
                    ...
                    print(
                        f"[ARCHIVED DEBUG] Column {col}: {mode_name}, width: {width}px"
                    )

                print(
                    f"[ARCHIVED DEBUG] Total table width: {total_width}px, Available: {table.viewport().width()}px"
                )
    ...
    def _force_proportional_distribution(self, table):
        ...
        try:
            ...
            print(
                f"[ARCHIVED DEBUG] Forcing proportional distribution with viewport width: {available_width}px"
            )
            ...
            for col in range(table.columnCount()):
                ...
                print(
                    f"[ARCHIVED DEBUG]   {header_text}: {current_width}px -> {new_width}px (target: {min_width}px, weight: {weight:.2f})"
                )
            ...
            print(
                f"[ARCHIVED DEBUG] Forced proportional distribution with weights: {column_weights}"
            )
    ...
    def _get_cached_font_metrics_for_column(
        self, table: SmoothTableWidget, col: int, header_text: str
    ):
    ...
    def _invalidate_table_cache_on_data_change(self):
        ...
        try:
            if (
                ...
                and hasattr(self, "archived_records_table")
            ):
                self._cache_manager.invalidate_cache_for_table("archive_table")
        ...
        try:
            ...
            # Always log to console if debug enabled (for immediate feedback)
                ...
                if duration is not None:
                    ...
                    print(
                        f"[{timestamp}] ARCHIVED RESIZE: {operation} - {duration:.2f}ms [{status}]"
                    )
                else:
        ...
        try:
            if hasattr(self, "_cache_manager") and self._cache_manager:
                ...
                stats["table_info"] = {
                    ...
                    "current_row_count": self.archived_records_table.rowCount()
                    if hasattr(self, "archived_records_table")
                    and self.archived_records_table
                    else 0,
                    "current_column_count": self.archived_records_table.columnCount()
                    if hasattr(self, "archived_records_table")
                    and self.archived_records_table
                    else 0,
                }
    ...
    def debug_column_width_distribution(self):
        ...
        try:
            if (
                not hasattr(self, "archived_records_table")
                or not self.archived_records_table
            ):
    ...
    def _apply_cached_column_widths(
        self, table: SmoothTableWidget, column_widths: List[int]
    ):
    ...
    def show_record_details_dialog(self, index):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Error", "Unable to open record details. Please try again."
            )
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, "archived_records_table") and self.archived_records_table:
            QTimer.singleShot(
                50, lambda: self._apply_equal_column_widths(self.archived_records_table)
            )
    ...
    def showEvent(self, event):
        ...
        try:
            ...
            if hasattr(self, "archived_records_table") and self.archived_records_table:
                QTimer.singleShot(
                    100,
                    lambda: self._apply_equal_column_widths(
    ...
    def _recalculate_all_table_widths(self):
        ...
        try:
            ...
            if (
                ...
                if (
                    hasattr(self, "archived_records_table")
                    and self.archived_records_table
                ):
                    self._set_intelligent_column_widths(self.archived_records_table)
                    return True
        ...
        try:
            ...
            if hasattr(self, "archived_records_table") and self.archived_records_table:
                self._optimization_manager.setup_table_optimization(
                    self.archived_records_table, "archive_table"
                )
    ...
    def _perform_debounced_resize(self):
        ...
        try:
            ...
            if (
                start_time
                and hasattr(self, "_debug_manager")
                and self._debug_manager
                and self._debug_manager.enabled
            ):
    ...
    def _emergency_resize_fallback(self):
        ...
        try:
            if hasattr(self, "archived_records_table") and self.archived_records_table:
                ...
                try:
                    header.setStretchLastSection(True)
                    self.archived_records_table.setHorizontalScrollBarPolicy(
    ...
    def enable_optimization_safe_mode(self):
        ...
        try:
            if self._optimization_manager:
                ...
                print(
                    "[ARCHIVED TAB] Cannot enable safe mode - optimization manager not available"
                )
        except Exception as e:
    ...
    def reset_optimization_state(self):
        ...
        try:
            if self._optimization_manager:
                ...
                self._setup_optimization_components()
                print(
                    "[ARCHIVED TAB] Attempted to reinitialize optimization components"
                )
        except Exception as e:
            ...
            if hasattr(self, "archived_records_table") and self.archived_records_table:
                ...
                def attempt_resize(attempt=1):
                    try:
                        ...
                        print(
                            f"[ARCHIVED FORCE REFRESH] Attempt {attempt}: Viewport width = {viewport_width}px"
                        )
                        ...
                        if viewport_width > 50:
                            ...
                            print(
                                f"[ARCHIVED FORCE REFRESH] Success on attempt {attempt}"
                            )
                        elif attempt < 5:
                            ...
                            print(
                                f"[ARCHIVED FORCE REFRESH] Failed after {attempt} attempts"
                            )
                    except Exception as e:
                        print(
                            f"[ARCHIVED FORCE REFRESH] Error on attempt {attempt}: {e}"
                        )
    ...
    def _is_numeric_column(self, header_text: str) -> bool:
        ...
        numeric_indicators = ["advanced paid"]
        header_lower = header_text.lower()
